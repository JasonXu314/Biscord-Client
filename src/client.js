...
{
    let port;
    const connection = new WebSocket('ws://biscord-server.herokuapp.com:3000/');
    connection.addEventListener('message', (msg) => {
        if (JSON.parse(msg.data).type === 'port')
        {
            connection.close();
            port = JSON.parse(msg.data).port;
            connection = new WebSocket(`ws://biscord-server.herokuapp.com:${port}/`);
